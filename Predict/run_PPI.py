...
class Simple_MLP(nn.Module):
    ...
    def __init__(self, config):
        ...
        classifier_dropout = config.cls_dropout_prob
        ...
        self.dense2 = nn.Linear(config.hidden_size //4 , config.hidden_size // 8)
        self.dense3 = nn.Linear(config.hidden_size // 8, config.hidden_size // 32)
        self.dropout = nn.Dropout(classifier_dropout)
        self.activation = nn.ReLU()
        self.num_labels = config.num_labels
        self.out_proj = nn.Linear(config.hidden_size // 32, config.num_labels)
        self.config = config
...
#         device = x.device
#         weight_CE = torch.tensor([6.035],dtype=torch.float32, device=device)
#         loss_fct = BCEWithLogitsLoss(pos_weight = weight_CE)
...
# import sys
# sys.path.append('/data2/jyguo/ProtRB/src/')
...
###### 1. data & files
    ...
    parser.add_argument("--save_path", type=str, default=None, help="Where to store the final model.")
...
#     parser.add_argument("--hidden_size", type=int, required=True, help="Input layer size of the HMCN net.")
    parser.add_argument("--label_num", type=int, help="Predict mode label numbers.")
...
def main():
    ...
    def preprocess_multilabel_function(examples):
        ...
        if "PPI_features" in examples:
            result["features"] = list(map(lambda l:Translate_feature(l), examples['PPI_features']))
    ...
    model_class, args.feature_dim, args.hidden_size = Simple_MLP, 2891, 1024
...
#             logger.info(f"Sample {index} of the training set: {train_dataset[index]}.")
    ...
    if args.do_train:
        ...
        f.write(f'Model_{args.per_device_train_batch_size}_{args.learning_rate}_{args.dropout_prob}_{args.cv_num}:\t')
